...
import org.zkoss.zsoup.parser.ParseError;
...
public class HtmlParserTest {
    ...
    @Test public void parsesUnterminatedTextarea() {
        ...
        Document doc = Zsoup.parse("<body><p><textarea>one<p>two");
    ...
    @Test public void parsesUnterminatedOption() {
        ...
        Document doc = Zsoup.parse("<body><p><select><option>One<option>Two</p><p>Three</p>");
    ...
    @Test public void testSpaceAfterTag() {
        Document doc = Zsoup.parse("<div > <a name=\"top\"></a ><p id=1 >Hello</p></div>");
    ...
    @Test public void handlesTextAfterData() {
        ...
        Document doc = Zsoup.parse(h);
        assertEquals("<html><head></head><body>pre <script>inner</script> aft</body></html>", TextUtil.stripNewlines(doc.html()));
    }

    @Test public void handlesTextArea() {
        Document doc = Zsoup.parse("<textarea>Hello</textarea>");
    ...
    @Test public void preservesSpaceInTextArea() {
        ...
        Document doc = Zsoup.parse("<textarea>\n\tOne\n\tTwo\n\tThree\n</textarea>");
    ...
    @Test public void preservesSpaceInScript() {
        ...
        Document doc = Zsoup.parse("<script>\nOne\n\tTwo\n\tThree\n</script>");
        String expect = "\nOne\n\tTwo\n\tThree\n";
    ...
    @Test public void handlesNestedImplicitTable() {
        Document doc = Zsoup.parse("<table><td>1</td></tr> <td>2</td></tr> <td> <table><td>3</td> <td>4</td></table> <tr><td>5</table>");
    ...
    @Test public void handlesWhatWgExpensesTableExample() {
        ...
        Document doc = Zsoup.parse("<table> <colgroup> <col> <colgroup> <col> <col> <col> <thead> <tr> <th> <th>2008 <th>2007 <th>2006 <tbody> <tr> <th scope=rowgroup> Research and development <td> $ 1,109 <td> $ 782 <td> $ 712 <tr> <th scope=row> Percentage of net sales <td> 3.4% <td> 3.3% <td> 3.7% <tbody> <tr> <th scope=rowgroup> Selling, general, and administrative <td> $ 3,761 <td> $ 2,963 <td> $ 2,433 <tr> <th scope=row> Percentage of net sales <td> 11.6% <td> 12.3% <td> 12.6% </table>");
    ...
    @Test public void handlesTbodyTable() {
        Document doc = Zsoup.parse("<html><head></head><body><table><tbody><tr><td>aaa</td><td>bbb</td></tr></tbody></table></body></html>");
    ...
    @Test public void handlesImplicitCaptionClose() {
        Document doc = Zsoup.parse("<table><caption>A caption<td>One<td>Two");
    ...
    @Test public void noTableDirectInTable() {
        Document doc = Zsoup.parse("<table> <td>One <td><table><td>Two</table> <table><td>Three");
    ...
    @Test public void ignoresDupeEndTrTag() {
        Document doc = Zsoup.parse("<table><tr><td>One</td><td><table><tr><td>Two</td></tr></tr></table></td><td>Three</td></tr></table>"); // two </tr></tr>, must ignore or will close table
    ...
    @Test public void handlesBaseTags() {
        ...
        Document doc = Zsoup.parse(h, "http://foo/");
    ...
    @Test public void handlesUnknownInlineTags() {
        ...
        Document doc = Zsoup.parseBodyFragment(h);
    ...
    @Test public void parsesBodyFragment() {
        ...
        Document doc = Zsoup.parseBodyFragment(h, "http://example.com");
    ...
    @Test public void handlesBaseWithoutHref() {
        ...
        Document doc = Zsoup.parse(h, "http://example.com/");
        Element a = doc.select("a").first();
    ...
    @Test public void testHgroup() {
        ...
        Document doc = Zsoup.parse("<h1>Hello <h2>There <hgroup><h1>Another<h2>headline</hgroup> <hgroup><h1>More</h1><p>stuff</p></hgroup>");
    ...
    @Test public void testRelaxedTags() {
        Document doc = Zsoup.parse("<abc_def id=1>Hello</abc_def> <abc-def>There</abc-def>");
    ...
    @Test public void testHeaderContents() {
        ...
        Document doc = Zsoup.parse("<h1>Hello <div>There</div> now</h1> <h2>More <h3>Content</h3></h2>");
    ...
    @Test public void testSpanContents() {
        ...
        Document doc = Zsoup.parse("<span>Hello <div>there</div> <span>now</span></span>");
    ...
    @Test public void testNoImagesInNoScriptInHead() {
        ...
        Document doc = Zsoup.parse("<html><head><noscript><img src='foo'></noscript></head><body><p>Hello</p></body></html>");
    ...
    @Test public void testAFlowContents() {
        ...
        Document doc = Zsoup.parse("<a>Hello <div>there</div> <span>now</span></a>");
    ...
    @Test public void testFontFlowContents() {
        ...
        Document doc = Zsoup.parse("<font>Hello <div>there</div> <span>now</span></font>");
    ...
    @Test public void handlesUnclosedTitleAtEof() {
        assertEquals("Data", Zsoup.parse("<title>Data").title());
        assertEquals("Data<", Zsoup.parse("<title>Data<").title());
        assertEquals("Data</", Zsoup.parse("<title>Data</").title());
        assertEquals("Data</t", Zsoup.parse("<title>Data</t").title());
        assertEquals("Data</ti", Zsoup.parse("<title>Data</ti").title());
        assertEquals("Data", Zsoup.parse("<title>Data</title>").title());
        assertEquals("Data", Zsoup.parse("<title>Data</title >").title());
    }

    @Test public void handlesUnclosedTitle() {
        Document one = Zsoup.parse("<title>One <b>Two <b>Three</TITLE><p>Test</p>"); // has title, so <b> is plain text
        assertEquals("One <b>Two <b>Three", one.title());
        assertEquals("Test", one.select("p").first().text());

        Document two = Zsoup.parse("<title>One<b>Two <p>Test</p>"); // no title, so <b> causes </title> breakout
        assertEquals("One", two.title());
        assertEquals("<b>Two <p>Test</p></b>", two.body().html());
    }

    @Test public void handlesUnclosedScriptAtEof() {
        assertEquals("Data", Zsoup.parse("<script>Data").select("script").first().data());
        assertEquals("Data<", Zsoup.parse("<script>Data<").select("script").first().data());
        assertEquals("Data</sc", Zsoup.parse("<script>Data</sc").select("script").first().data());
        assertEquals("Data</-sc", Zsoup.parse("<script>Data</-sc").select("script").first().data());
        assertEquals("Data</sc-", Zsoup.parse("<script>Data</sc-").select("script").first().data());
        assertEquals("Data</sc--", Zsoup.parse("<script>Data</sc--").select("script").first().data());
        assertEquals("Data", Zsoup.parse("<script>Data</script>").select("script").first().data());
        assertEquals("Data</script", Zsoup.parse("<script>Data</script").select("script").first().data());
        assertEquals("Data", Zsoup.parse("<script>Data</script ").select("script").first().data());
        assertEquals("Data", Zsoup.parse("<script>Data</script n").select("script").first().data());
        assertEquals("Data", Zsoup.parse("<script>Data</script n=").select("script").first().data());
        assertEquals("Data", Zsoup.parse("<script>Data</script n=\"").select("script").first().data());
        assertEquals("Data", Zsoup.parse("<script>Data</script n=\"p").select("script").first().data());
    }

    @Test public void handlesUnclosedRawtextAtEof() {
        assertEquals("Data", Zsoup.parse("<style>Data").select("style").first().data());
        assertEquals("Data</st", Zsoup.parse("<style>Data</st").select("style").first().data());
        assertEquals("Data", Zsoup.parse("<style>Data</style>").select("style").first().data());
        assertEquals("Data</style", Zsoup.parse("<style>Data</style").select("style").first().data());
        assertEquals("Data</-style", Zsoup.parse("<style>Data</-style").select("style").first().data());
        assertEquals("Data</style-", Zsoup.parse("<style>Data</style-").select("style").first().data());
        assertEquals("Data</style--", Zsoup.parse("<style>Data</style--").select("style").first().data());
    }
    ...
    @Test public void tracksErrorsWhenRequested() {
        ...
        Document doc = Zsoup.parse(html, "http://example.com", parser);
    ...
    @Test public void handlesCommentsInTable() {
        ...
        Document node = Zsoup.parseBodyFragment(html);
